...
data = np.load('./data/DIY_origin.npy')
...
class MLP(object):
	...
	def judge(self, y_hat, y):
		"""
		if y_hat > 0.5, return 1, else return 0
...
train_data = np.random.randn(30000, 15)
train_label = np.random.randint(0, 2, size=(30000,1))
train = np.hstack((train_data, train_label))
test_data = np.random.randn(3000, 15)
test_label = np.random.randint(0, 2, size=(3000,1))
test = np.hstack((test_data, test_label))

model = MLP(train, test, train.shape[1]-1, 1, 0.5, 10000)
model.train_model()
print('\n\n')
